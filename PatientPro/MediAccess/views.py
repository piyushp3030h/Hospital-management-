...
from .forms import CustomUserCreationForm, LoginForm, DoctorUpdateForm, PatientRecordForm, PatientUpdateForm
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save(commit=False)  # Delay saving so we can modify the user
            user.set_password(form.cleaned_data['password1'])
            # Doctor approval logic
            if user.user_type == 'doctor':
                user.is_active = False  # Mark doctor account as inactive
                messages.info(request, 'Your account is under review. Admin will approve it shortly.')

            user.save()  # Save user after potential modification

            # Only auto-login if the account is active
            if user.is_active:
                auth_login(request, user, backend='MediAccess.backends.MyCustomBackend')
                messages.success(request, 'Registration successful.')
                return redirect('login')
            else:
                return redirect('login')  # Redirect even if not active (with info message)
        else:
...
def download_medicine_description(request):
    if request.user.is_authenticated and request.user.user_type == 'patient':
        try:
            record = PatientRecord.objects.get(patient=request.user)
            if record.medicine_description:
                response = HttpResponse(record.medicine_description, content_type='text/plain')
                response['Content-Disposition'] = 'attachment; filename="medicine_description.txt"'
                return response
            else:
                messages.warning(request, "No medicine description found.")
                return redirect('view_patient_record')
